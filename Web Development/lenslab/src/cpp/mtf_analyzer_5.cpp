...
public:
    ...
    static EdgeProfileData sampleEdgeProfiles(const cv::Mat& roi, const EdgeROI& edge_data, bool debug = false)
    {
        ...
        result.profiles.erase(
            std::remove_if(result.profiles.begin(), result.profiles.end(),
                        [](const std::vector<double>& profile) { return profile.empty(); }),
            result.profiles.end()
        );
    ...
    static ESFData computeSuperResolutionESF(const EdgeProfileData& profiles, double angle_degrees, bool debug = false)
    {
        ...
        // Pre-allocate vectors with calculated capacity
        result.distances.reserve(total_points);
        result.intensities.reserve(total_points);
        
        // Estimate binned data size (typically smaller than raw data)
        // Using an estimated compression ratio of 10:1 for binning
        size_t estimated_bin_count = total_points / 10;
        result.binned_distances.reserve(estimated_bin_count);
        result.binned_intensities.reserve(estimated_bin_count);
        ...
        for (size_t profile_idx = 0; profile_idx < profiles.profiles.size(); profile_idx++) {
            ...
            // Pre-allocate gradients vector
            std::vector<double> gradients;
            gradients.reserve(profile.size() - 1);
            
            // Calculate gradients for edge detection
            for (size_t i = 0; i < profile.size() - 1; i++) {
                gradients.push_back(std::abs(profile[i + 1] - profile[i]));
            }
        ...
        // Apply moving average with pre-allocated vector
        ...
        // Pre-allocate vectors in bins (optional, might help if bin distribution is somewhat uniform)
        size_t estimated_points_per_bin = total_points / estimated_bin_count;
        
        for (size_t i = 0; i < result.distances.size(); i++) {
            int bin = static_cast<int>((result.distances[indices[i]] - min_dist) / BIN_WIDTH);
            if (bins[bin].empty()) {
                bins[bin].reserve(estimated_points_per_bin);
            }
